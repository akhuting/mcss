...
$(document).ready(function() {
    ...
    $("#nav > li > a").click(function() {
        ...
        if ($(this).attr("id") == "sub") {
            var html = $(this).text();
            $.each($(".expanded"), function(i, obj) {
//                if ($(obj).text() != "数据分析") {
//                    if ($(obj).text() != html) {
//                        $(obj).toggleClass('expanded').toggleClass('collapsed').parent().find('> ul').slideToggle('medium');
//                    }
//                }
            });
            $(this).toggleClass('expanded').toggleClass('collapsed').parent().find('> ul').slideToggle('medium');
        } else if ($(this).attr("className").indexOf("expanded") == -1) {
            $(".expanded").toggleClass().toggleClass("collapsed").parent().find('> ul').slideToggle('medium');
            $(this).toggleClass('expanded').toggleClass('collapsed').parent().find('> ul').slideToggle('medium');
        } else {
            $(".expanded").toggleClass().toggleClass("collapsed").parent().find('> ul').slideToggle('medium');
        }
...
(function(a) {
    a.fn.bPopup = function(f, j) {
        function s() {
            ...
            c.css({left:d.scrollLeft() + h,position:"absolute",top:k,"z-index":o.zIndex}).appendTo(o.appendTo).hide(
                    function() {
                        b && c.each(function() {
                        ...
                        if (o.loadUrl != null) {
                            ...
                            switch (o.content) {
                                ...
                                case "iframe":
                                    ...
                                    break;
                                case "xlink":
                                    a("a#bContinue").attr({href:o.loadUrl});
        ...
        function u() {
            if (m) {
                ...
                return{"background-color":o.modalColor,height:b[0],left:l(),opacity:0,position:"absolute",top:0,width:b[1],"z-index":o.zIndex - 1}
            } else return{"background-color":o.modalColor,height:"100%",left:0,opacity:0,position:"fixed",top:0,width:"100%","z-index":o.zIndex - 1}
        }
        ...
        function t() {
            ...
            o.follow && e.bind("scroll.bPopup",
                    function() {
                        c.stop().animate({left:d.scrollLeft() + h,top:d.scrollTop() + g}, o.followSpeed)
                    }).bind("resize.bPopup", function() {
                        if (o.modal && m) {
                            var b = [d.height(),d.width()];
                            n.css({height:b[0],width:b[1],left:l()})
                        }
                        b = p(c, o.amsl);
                        g = b[0];
                        h = b[1];
        ...
        return this.each(function() {
            if (!c.data("bPopup")) {
                o.modal && n.css(u()).appendTo(o.appendTo).animate({opacity:o.opacity}, o.fadeSpeed);
                c.data("bPopup", o);
                s()
            }
        })
